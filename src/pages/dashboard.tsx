...
import { getAttendanceRecords } from 'apiSdk/attendance-records';
...
function DashboardPage() {
  ...
  const departmentStat = useSWR<PaginatedInterface<DepartmentInterface>>(
    () => `/departments`,
    () => getDepartments({ limit: 1, offset: 0 }),
  );

  const attendanceRecordStat = useSWR<PaginatedInterface<AttendanceRecordInterface>>(
    () => `/attendance-records`,
    () => getAttendanceRecords({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    {
      title: 'Departments',
      result: departmentStat?.data?.totalCount,
      isLoading: departmentStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Attendance Records',
      result: attendanceRecordStat?.data?.totalCount,
      isLoading: attendanceRecordStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
