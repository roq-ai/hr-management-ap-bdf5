...
type ColumnType = ColumnDef<EmployeeTrainingInterface, unknown>;

interface EmployeeTrainingListPageProps {
...
export function EmployeeTrainingListPage(props: EmployeeTrainingListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<EmployeeTrainingInterface>>(
    () => `/employee-trainings?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: EmployeeTrainingInterface) => {
    if (hasAccess('employee_training', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/employee-trainings/view/${row.id}`);
    }
  };
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/employee-trainings/view/${record.id}`} passHref legacyBehavior>
                <Button
    ...
    <Flex direction="column" gap={{ md: 6, base: 7 }} shadow="none">
      <Flex justifyContent={{ md: 'space-between' }} direction={{ base: 'column', md: 'row' }} gap={{ base: '28px' }}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="employee_training" />
        </Flex>
